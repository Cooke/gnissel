...
{
    public static Setter CreateSetter<TTable, TProperty>(
        Table<TTable> table,
        Expression<Func<TTable, TProperty>> propertySelector,
        TProperty value
    ) => new ValueSetter<TProperty>(FindColumn(table, propertySelector), value);

    public static Setter CreateSetter<TTable, TProperty>(
        Table<TTable> table,
        Expression<Func<TTable, TProperty>> propertySelector,
        Expression<Func<TTable, TProperty>> value
    ) =>
        new ExpressionSetter(
            FindColumn(table, propertySelector),
    ...
    private static Column<TTable> FindColumn<TTable, TProperty>(
        ...
        table.Columns.FirstOrDefault(x =>
            x.MemberChain.SequenceEqual(ExpressionUtils.GetMemberChain(columnSelector.Body))
